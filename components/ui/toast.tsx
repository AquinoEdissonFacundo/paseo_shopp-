...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[100] flex max-h-screen w-full max-w-[90vw] sm:max-w-[420px] flex-col-reverse p-0 sm:bottom-0 sm:right-0 sm:left-auto sm:top-auto sm:translate-x-0 sm:translate-y-0 sm:flex-col',
      className,
    )}
    {...props}
  />
...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border-2 p-6 pr-8 shadow-2xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        default: 'border-primary/50 bg-primary text-primary-foreground backdrop-blur-sm',
        success: 'border-[#14B8A6]/50 bg-gradient-to-r from-[#14B8A6] to-[#027373] text-white shadow-[0_0_20px_rgba(20,184,166,0.4)]',
        destructive:
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn('text-sm font-semibold group-[.success]:text-white', className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn('text-sm opacity-90 group-[.success]:text-white/95', className)}
    {...props}
  />
))
